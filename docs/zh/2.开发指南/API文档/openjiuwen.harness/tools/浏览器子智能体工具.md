...
可用时，每个元素还包含 `ax` 对象，其中提供 Playwright 计算的 `role`、无障碍 `name`、`checked` / `disabled` / `expanded` / `invalid` / `pressed` / `selected` 等适用状态，以及当前表单 `value`。这些信息在同一次 run-code 调用中确定性计算，不会捕获整页快照，也不会返回 Playwright 原生 ref。顶层 `ax_enrichment` 会报告 `complete`、`partial` 或 `unavailable`；AX 获取失败不会使可用的 DOM 探测失败。
